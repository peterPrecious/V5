<%
  ...
  Sub sGetQueryString
    ...
    End If
    '...check language
'   If Instr(" ROOT EN FR ES ", vLang) = 0 Then 
'     vLang = ""
'   End If
'   Session("Lang") = vLang
'   svLang = vLang
  ...
  Sub sPutQueryString
    Session("QueryString") =  "vHostDb=" & vHostDb & "&vCust=" & vCust & "&vId=" & vId & "&vPwd=" & vPwd & "&vLang=" & vLang & "&vFirstName=" & vFirstName & "&vLastName=" & vLastName & "&vEmail=" & vEmail & "&vMemo=" & vMemo & "&vCriteria=" & vCriteria & "&vSource=" & vSource & "&vQModId=" & vQModId & "&vQTestId=" & vQTestId & "&vGoTo=" & vGoTo & "&v8=" & v8 & "&zoneoffset=" & vZoneoffset & "&platform=" & vPlatform & "&browser=" & vBrowser & "&bver=" & vBVer & "&flashver=" & vFlashVer & "&vAction=" & vAction  & "&vTskH_Id=" & vTskH_Id
  End Sub  
