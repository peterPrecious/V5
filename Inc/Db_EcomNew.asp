<%
  ...
  Sub sAddEcom ()
    ...
    vSql = vSql & " VALUES ('" & vEcom_CustId & "', '" & vEcom_AcctId & "', '" & vEcom_Id & "', " & vEcom_MembNo & ", " & fDefault(vEcom_CatlNo, 0) & ", '" & fIf(Len(vEcom_Programs) > 7, Right(vEcom_Programs, 7), vEcom_Programs) & "', " & vEcom_Prices & ", " & vEcom_Taxes & ", '" & vEcom_Issued & "', '" & vEcom_Expires & "', " & vEcom_Amount & ", '" & vEcom_Currency & "', '" & vEcom_Lang & "', '" & vEcom_FirstName & "', '" & vEcom_LastName & "', '" & vEcom_CardName & "', '" & vEcom_Address & "', '" & vEcom_City & "', '" & vEcom_Postal & "', '" & vEcom_Province & "', '" & vEcom_Country & "', '" & vEcom_Phone & "', '" & vEcom_Email & "', " & vEcom_Quantity & ", '" & vEcom_NewAcctId & "', '" & vEcom_Media & "', '" & vEcom_Label & "', '" & vEcom_OrderNo & "', " & vEcom_Shipping & ", '" & vEcom_Memo & "', '" & vEcom_Organization & "', '" & fDefault(vEcom_Source,"E") & "', '" & vEcom_InternetSecure & "', '" & vEcom_Agent & "')"
'   sDebug
    sOpenDb
    oDb.Execute(vSql)
    sCloseDb
  ...
  Function fEcomPrograms (vCustId, vMembId)
    Dim aPrograms
    fEcomPrograms = ""
    vSql = "SELECT Ecom_Programs, Ecom_Expires FROM Ecom WHERE Ecom_CustId = '" & vCustId & "' AND Ecom_Id= '" & vMembId & "' AND Ecom_Expires >= '" & fFormatSqlDate(Now) & "'"
'   sDebug
    sOpenDb    
  ...
  Function fEcomProgram2 (vCustId, vMembId)
    Dim aPrograms, i
    fEcomProgram2 = ""
    vSql = "SELECT Ecom_Programs FROM Ecom WHERE Ecom_CustId = '" & vCustId & "' AND Ecom_Id= '" & vMembId & "' AND Ecom_Expires >= '" & fFormatSqlDate(Now) & "'"
'   sDebug
    sOpenDb3
  ...
  Sub sGetEcom_Rs (vEcomCustId, vEcomMembId)
    vSql = "SELECT * FROM Ecom WHERE Ecom_CustId = '" & vEcomCustId & "' AND Ecom_Id= '" & vEcomMembId & "'"
    sOpenDb    
    Set oRs = oDb.Execute(vSql)
  End Sub
  ...
  Sub sGetEcomByNewAcctId_Rs (vEcomNewAcctId)
    vSql = "SELECT * FROM Ecom WHERE Ecom_NewAcctId = '" & vEcomNewAcctId & "'"
    sOpenDb    
    Set oRs = oDb.Execute(vSql)
  End Sub
  ...
  Function fEcomCustId (vEcomNewAcctId)
    vSql = "SELECT DISTINCT Ecom_CustId FROM Ecom WHERE Ecom_NewAcctId = '" & vEcomNewAcctId & "'"
    sOpenDb    
    Set oRs = oDb.Execute(vSql)
    If oRs.Eof Then 
      fEcomCustId = ""
...
'   vSql = "SELECT Ecom_Programs, SUM(Ecom_Quantity) AS Ecom_Quantity, Ecom_Expires FROM Ecom WHERE Ecom_NewAcctId = '" & svCustAcctId & "' AND Ecom_Quantity > 0 GROUP BY Ecom_Programs, Ecom_Expires ORDER BY Ecom_Programs, Ecom_Expires"
...
'     fEcomGroupProgs = fEcomGroupProgs  & vbCrLf & "          <input type='hidden' name='vMemb_Expires' value='" & fFormatDate(vEcom_Expires) & "'>"
