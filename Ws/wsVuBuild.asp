 ...
 <%vBypassSecurity = True
  Session("Host")      = Lcase(Request.ServerVariables("HTTP_HOST") & "/V5")
...
<%
  ...
  Sub sReadCust
    ...
    vResponseXML =  "    <Customer>" _
                  ...
                  & "      <vCust_Lang>"    & Server.HTMLEncode(oRs("Cust_Lang"))                       & "</vCust_Lang>"    _
                  & "      <vCust_Prgms>"   & Server.HTMLEncode(Left(oRs("Cust_Programs"), 7))          & "</vCust_Prgms>"   _
                  & "      <vCust_Active>"  & Server.HTMLEncode(oRs("Cust_Active"))                     & "</vCust_Active>"  _
                  & "      <vCust_Auth>"    & Server.HTMLEncode(oRs("Cust_Auth"))                       & "</vCust_Auth>"    

  End Sub  
  Sub sReadCustMods
       sOpenDbBase
       'vSql = "Select Prog_Mods, Prog_Title1, Prog_Desc FROM Prog WHERE Prog_Id = '" & vCust_Prgms & "'"
...
vSql = "Select Prog_Mods, Prog_Title1, Prog_Desc FROM Prog WHERE Prog_Id = '" & vCust_Prgms & "'"
       ...
       vResponseXML = "      <vCust_Mods>"        & Server.HTMLEncode(oRs("Prog_Mods"))    & "</vCust_Mods>"       _
                    & "      <vCust_Prog_Title>"  & Server.HTMLEncode(oRs("Prog_Title1"))  & "</vCust_Prog_Title>" _
  ...
  Sub sReadMemb
    vResponseXML =  "    <Member>" _
                  & "      <vMemb_AcctId>"    & Server.HTMLEncode(oRs("Memb_AcctId"))    & "</vMemb_AcctId>"      _
                  & "      <vMemb_No>"        & Server.HTMLEncode(oRs("Memb_No"))        & "</vMemb_No>"          _
                  ...
                  & "      <vMemb_Level>"     & Server.HTMLEncode(oRs("Memb_Level"))     & "</vMemb_Level>"       _
                  & "      <vMemb_Auth>"      & Server.HTMLEncode(oRs("Memb_Auth"))      & "</vMemb_Auth>"        _
                  & "    </Member>" 

  End Sub
    

  '... No Longer used, was for multiple programs
