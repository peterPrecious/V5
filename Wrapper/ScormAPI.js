  ...
  function API() {
    ...
    this.elements[18] = "cmi.interactions";
  	this.elements[19] = "cmi.student_data.attempt_max";
  	this.elements[20] = "cmi.student_data.attempt_number";
    ...
    this.dup_elements[18] = "cmi.interactions";
  	this.dup_elements[19] = "cmi.student_data.attempt_max";
  	this.dup_elements[20] = "cmi.student_data.attempt_number";
    ...
    this.values[18] = ""; 
    this.values[19] = ""; // "<%=Session("Attempt_Max")%>"; 
    this.values[20] = ""; // "<%=Session("Attempt_No")%>"; 
  ...
  function LMSFinish(arg) {
    ...
    if (vDebug_All) alert ("finishing API...");
  	...
  	if (this.APIInitialized) {
        ...
        // added by Peter Bulloch to commit if not already committed
  
        if (vCommitted = false) {
          if (vDebug_Key) alert('not committed...will force a commit');
          this.LMSCommit();
        } 
        else {
          if (vDebug_Key) alert ('committed...no need for forced commit');
        }
  ...
  { 
    if (vDebug_Key) alert ("SET API values: " + ele + " = " + val);
      ...
      if (Left(ele, 16) == "cmi.interactions") {
          ...
          if (vDebug_All) alert ("SET cmi.interactions: " + this.values[i]);
  ...
  function LMSCommit(arg){
    ...
    if (vDebug_All) alert("Checking to see if the API is initialized so we can move forward...");

  	if (this.APIInitialized) {

      if (vDebug_All) alert("It is initialized, check if args are OK...");
      if (vDebug_All) alert("arg = " + arg);
        ...
        if (vDebug_All) alert("Checking to see if we can create a form to send info to the LMS...");
       
  
      	if (create_submitForm(this.dup_elements,this.values)) {
          if (vDebug_Key) alert("Creating and Sending the form to the LMS...");
      		header.document.getElementById('submitForm').submit();
      	}
  ...
  function create_submitForm(dup_ele_arr, val_arr){
    ...
    var submitForm = "";
    submitForm += '\n<form name="submitForm" id="submitForm" action="/V5/ScoreModule.asp" method="post">';
    ...
    if (vDebug_Key) alert("Confirming comit");
  	if (header.document.getElementById('scorm')){
  		header.document.getElementById('scorm').innerHTML = submitForm;
      vCommitted = true;
  		return true;
  	}
  	return false;
