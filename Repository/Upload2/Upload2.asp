...
<head>
  <title>Upload2</title>
  <meta charset="UTF-8">
  <script src="/V5/Scripts/jquery.js"></script>
...
<body>
  ...
  <% 
    ...
    Dim oUp, oFs, vFileName, vFileNameOk, vSize, vCustId, vAction, vDaysOk, vAddProgs, vUseGroups
    ...
    If oUp.Form.Count > 0 Then
      ...
      If Len(vFileName) = 0 Then
        Response.Redirect "/V5/Code/Error.asp?vCustId=" & vCustId & "&vAction=" & vAction & "&vDaysOk=" & vDaysOk & "&vAddProgs=" & vAddProgs & "&vUseGroups=" & vUseGroups & "&vReturn=/V5/Repository/Upload2/Upload2.asp&vErr=" & Replace("No file has been selected.", " ", "+")
      End If
      vFileName = Ucase(Mid(vFileName, InstrRev(vFileName, "\") + 1))
      If vFileName <> vFileNameOk Then
        Response.Redirect "/V5/Code/Error.asp?vCustId=" & vCustId & "&vAction=" & vAction & "&vDaysOk=" & vDaysOk & "&vAddProgs=" & vAddProgs & "&vUseGroups=" & vUseGroups & "&vReturn=/V5/Repository/Upload2/Upload2.asp&vErr=" & Replace("Please browse to find the file named '" & vFileNameOk & "'.", " ", "+")
      End If
      vSize = oUp.TotalBytes

      '...delete file if it exists (Server 2008/IIS7 issue)
      If oFs.FileExists(Server.MapPath(vFileName)) Then oFs.DeleteFile (Server.MapPath(vFileName))

      oUp.SaveInVirtual vFileName
      Set oUp = Nothing
      If Err = 0 Then 
        Response.Redirect "Upload2_Ok.asp?vCustId=" & vCustId & "&vAction=" & vAction &  "&vDaysOk=" & vDaysOk & "&vAddProgs=" & vAddProgs 
      Else
        Response.Redirect "/V5/Code/Error.asp?vCustId=" & vCustId & "&vAction=" & vAction &  "&vDaysOk=" & vDaysOk & "&vAddProgs=" & vAddProgs & "&vUseGroups=" & vUseGroups & "&vReturn=/V5/Repository/Upload2/Upload2.asp&vErr=" & Replace("Your file could not be uploaded because:<br><br>" & Err.Description & ".", " ", "+")
      End If
  ...
  <form id="fUpload" enctype="multipart/form-data" method="post" action="Upload2.asp">
      ...
      <tr>
        <td style="text-align: center">
          ...
          <div id="divDetails" class="div" style="text-align: left;">
            <br>You can upload any number of Learner profiles from a <b>Tab Delimited Text File</b> stored on your system as <b><%=vCust_Id%>_LEARNERS.txt</b> (not case sensitive).&nbsp; <b>Do not upload a comma separated values (CSV) file</b>. New learner profiles will be added to the account while existing learners will have their profile updated accordingly. All profiles uploaded are deemed to be &quot;Learners&quot;, i.e. you cannot upload Facilitators, Managers, etc.<br><br>If you wish, you can <a href="Download2.asp">click here</a> and the system will extract <b>ALL Active Learners</b> from your site which you can save on your desktop as <b><%=vCust_Id%>_LEARNERS.xls</b> and use that to create a tab delimited file for uploading.<br><br>
            ...
            <div style="text-align: right">
              <table>
                ...
                <tr>
                  ...
                  <td><%=fIf(vCust_Pwd, "Mandatory", "Not used")%></td>
          ...
          <table id="meat">
            ...
            <tr>
              <td class="c2">2.</td>
              <td>
                If you are uploading <b>ALL</b> active learners, then you can select <b>ONE </b>of the following...<br><br>
                ...
                <input type="radio" name="vAction" value="s" <%=fcheck("s", vAction)%>>Inactive all existing Learners before uploading,<span class="red">except those that have been added within the last
                  <input type="text" name="vDaysOk" value="<%=vDaysOk%>" style="width: 20px" id="vDaysOk" />
                  days.</span><br>
            ...
            <tr>
              <td class="c2">3.</td>
              <td>
                If you Upload Program Ids, then specify...<br>
                <br>
            ...
            <tr>
              <td class="c2">4.</td>
              <td>
                Click <b>Submit</b> to upload the file...<br /><br>
