...
<%
  ...
  Dim vCustId, vAction, vDaysOk, vAddProgs, vUseGroups
  ...
  Sub sMessage (vError)
    ...
    If vRow > 1 Then
	    ...
	    vErr = vErr & "</table>"
	  End If
    Response.Redirect "/V5/Code/Error.asp?vCustId=" & vCustId & "&vAction=" & vAction & "&vDaysOk=" & vDaysOk & "&vAddProgs=" & vAddProgs & "&vUseGroups=" & vUseGroups & "&vReturn=/V5/Repository/Upload2/Upload2.asp&vErr=" & vErr
  End Sub
    ...
    If Len(Trim(Replace(vRecord, vbTab, ""))) > 0 Then
      ...
      If vRow > 1 Then '...skip header
        ...
        vCol = vCol + 1 : vMemb_Jobs        = Ucase(aRecord(vCol))        
				...
				spMembUpload2 vCust_AcctId, vMemb_Id, vMemb_Criteria, vMemb_FirstName, vMemb_LastName, vMemb_Email, vMemb_Pwd, vMemb_Programs, vMemb_Memo, vMemb_Jobs
      End If      
    End If
  Loop
...
' sCloseDb
  ...
  Function spMembUpload2 (vAcctId, vId, vCriteria, vFirstName, vLastName, vEmail, vPwd, vPrograms, vMemo, vJobs)
  	sOpenCmd
    With oCmd
      .CommandText = "spMembUpload2"
...
<body>
  ...
  <p style="text-align: center"><br /><br /><a class="c2" href="Upload2.asp">Restart Upload</a><%=f10%><a class="c2" href="/V5/Code/Users.asp">Learner List</a></p>
