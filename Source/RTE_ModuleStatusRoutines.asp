<%
  ...
  Function fModStatusLink (vMembNo, vProgId, vModId)
    ...
    vTitle = fModStatus (vMembNo, vModId) '...Status
    vTitle = vTitle
  ...
  Function fModStatus (vMembNo, vModId)
    If fCompleted (vMembNo, vModId) <> "" Then
      fModStatus  = Server.HtmlEncode("Completed") '...Completed
    ElseIf fTimeSpent(vMembNo, vModId) > 0 Then
      fModStatus  = Server.HtmlEncode("Reviewed") '...Reviewed
    Else
      fModStatus  = Server.HtmlEncode("Not Started") '...Not Started
    End If
  End Function


  '...return date module was either flagged as completed by V5 or status = "complete" by Scorm or assessment >= 80 over past 12 months
...
'   If Len(vModId) = 6 Then vModId = "undefined|" & vModId
  ...
  '...get best score of module assessment during the past 12 months
...
'     sDebug
  ...
  '...find total Number of Attempts at assessment
  ...
  Function fExamStatus (vMembNo, vExamString)
    fExamStatus = Server.HtmlEncode("<!--{{-->No Attempts<!--}}-->")
    Dim vModId, i, j
    ...
    If i > 0 Then 
      ...
      If j >= 0 Then fExamStatus = Server.HtmlEncode("<!--{{-->Best Score<!--}}-->") & ":&nbsp;" & j & "%"
      If (svBrowser <> "msie") Then
        fExamStatus = fExamStatus & " | <a onclick='location.reload();' href='#'>" & Server.HtmlEncode("<!--{{-->Refresh<!--}}-->") & "</a>"
      End If
    End If
  End Function  
  ...
  Function fAssessmentStatus(vMembNo, vModId)
    ...
    If k = 0 Then 
      fAssessmentStatus = Server.HtmlEncode("<!--{{-->No Attempts<!--}}-->")
    Else
      fAssessmentStatus = Server.HtmlEncode("<!--{{-->Best Score<!--}}-->") & ":&nbsp;" & j & "%"
      fAssessmentStatus = fAssessmentStatus & ",&nbsp;" & Server.HtmlEncode("<!--{{-->Attempts<!--}}-->") & ":&nbsp;" & fMin(k, i)
      If i < 99 Then
        fAssessmentStatus = fAssessmentStatus & "&nbsp;" & Server.HtmlEncode("<!--{{-->of<!--}}-->") & "&nbsp;" & i
      End If
      If j/100 >= fPassingScore Then 
        fAssessmentStatus = fAssessmentStatus & ", " & Server.HtmlEncode("<!--{{-->Completed<!--}}-->")
      End If
    End If  
    If (svBrowser <> "msie") Then
      fAssessmentStatus = fAssessmentStatus & " | <a onclick='location.reload();' href='#'>" & Server.HtmlEncode("<!--{{-->Refresh<!--}}-->") & "</a>"
    End If
  End Function  
