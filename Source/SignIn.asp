...
'  sMailServer
  ...
  If Len(vCust) <> 8 Then
    vError = "<!--{{-->the Customer Id is invalid.<!--}}-->"
...
'    Response.Redirect "SignInErr.asp?vClose=Y&vError=" & Server.UrlEncode(vError)
  ...
  If Not vCustOk Then
    ...
    If vMemb_Id = "" Then 
      vError = "<!--{{-->the Id and/or the Password is invalid.<!--}}-->"
      Response.Redirect "SignInErr.asp?vClose=Y&vError=" & Server.UrlEncode(vError)
    End If    
...
'     Case "QORDERPRODS"  : Response.Redirect "Ecom2BypassProds.asp"
  ...
  If vCustOk And fMembOk Then 
    ...
    If vAction = "ENROLL" Then
      Response.Redirect "Error.asp?vClose=y&vErr=" & Server.UrlEncode("<!--{{-->Welcome, you are now successfully enrolled!<!--}}-->") & "&vReturn=" & vSource
  ...
  Else
    ...
    If fNoValue(vError) Then vError = "<!--{{-->the Id and/or the Password is invalid.<!--}}-->"
    Response.Redirect "SignInErr.asp?vClose=Y&vError=" & Server.UrlEncode(vError)
  End If
  ...
  Function fMembOk
    ...
    If Not vMemb_Eof Then
      ...
      If vMemb_Level < 5 And vMemb_Active = 0 Then
        vError = "<!--{{-->your credentials have been inactivated.<!--}}-->"
        Exit Function
      End If     
      ...
      If vCust_IDsSize > 0 And Len(vId) <> vCust_IDsSize And vMemb_Sponsor = 0 Then
        vError = "<!--{{-->you are trying to enter this service without a valid Learner ID/Password.<!--}}-->"
        Exit Function
      End If
      ...
      If vCust_Auto And Len(vSource) = 0 And vAction <> "MYWORLD" And vAction <> "ECOMBYPASS" Then
        vError = "<!--{{-->for security reasons, you must sign in through the website where you originally gained access.<!--}}-->"
        Exit Function
      End If
      ...
      If vCust_Pwd And vAction <> "MYWORLD" Then
        ...
        If vAction = "ENROLL" Then
          vError = "<!--{{-->you have already Enrolled or you are trying to Enroll improperly.<!--}}-->"
          Exit Function
        ElseIf vAction <> "SIGNIN" THen
          vError = "<!--{{-->you are not properly configured to Signin or Enroll.<!--}}-->"
          Exit Function
        End If
        '...ensure pwd is valid
        If vPwd <> vMemb_Pwd Then
          vError = "<!--{{-->you have entered an invalid or missing Password.<!--}}-->"
          Exit Function
        End If
      End If      
    ...
    Else
      ...
      If vCust_Auto And Len(vSource) = 0 And vAction <> "MYWORLD" And vAction <> "ECOMBYPASS" Then
        vError = "<!--{{-->you must enter this service via the host site.<!--}}-->"
        Exit Function
      End If
      ...
      If vCust_Pwd Then
        ...
        If vAction = "SIGNIN" Then
          vError = "<!--{{-->you cannot SignIn with that Id.<!--}}-->"
          Exit Function
...
'    Next     
      ...
      If vMemb_Duration > 0 Then
        If Not IsDate(fFormatDate(vMemb_FirstVisit)) Then '...if no first date, make it today
          If Now > DateAdd("d", vMemb_Duration, Now) Then 
            vError = "<!--{{-->the number of available days to access this site has expired.<!--}}-->"
            Exit Function
          End If
  ...
  Function fCustOk
    ...
    If Not vCust_Active And vId <> vPassword5 Then 
      vError = "<!--{{-->that account is no longer active.<!--}}-->"
      Exit Function      
    End If
        ...
        If Now > vCust_Expires Then 
          If vId <> vPassword5 And vId <> "CCHSSALES" Then 
            ...
            If Not vMemb_Manager Then
              vError = "<!--{{-->access to that account has expired.<!--}}-->"
              Exit Function
            End If
          End If
        End If
