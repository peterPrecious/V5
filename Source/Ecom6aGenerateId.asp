...
<% 
  ...
  Dim bUpdateOnline, bUpdateGroup2, bTest, bCommit, b5Log, b5Step
  ...
  Dim logNo, dataIn, logGuid '...WS fields for logging

  '...is this a test? if so display results locally rather that to the web service
  bTest = fIf(Lcase(Request("vTest")) = "y", True, False)

  '...if this a test do we want to log? Always log otherwise
  b5Log = fIf(Lcase(Request("v5Log")) = "n", False, True)

  '...if we want to track where a transaction might fail, enable b5Step
  b5Step = fIf(Lcase(Request("v5Step")) = "n", False, True)
  ...
  '...return status info, display if testing

  '...if not empty then return error message
  If vStatus <> "" Then 
    If b5Log Then logGuid = fLogGuid()  '...need vStatus for log
    If bTest Then
      Response.Write vStatus & "<br>"
      If b5Log Then Response.Write "ResponseGUID" & "," & logGuid & "<br>"
    Else
      Response.Status = vStatus & "<br>"
      If b5Log Then Response.Write "ResponseGUID" & "," & logGuid
    End If

  '...send success plus values
  ...
  Else 
    ...
    If b5Log Then Response.AddHeader "ResponseGUID", logGuid
  ...
  '...log transaction (apps/ecomLogs)
  Function fLogGuid
    fLogGuid = ""
   	sOpenCmdApp
    With oCmdApp
      .CommandText = "sp5ecomLogs"
      .Parameters.Append .CreateParameter("@logNo",  	        adInteger, adParamInput,         , logNo)
      .Parameters.Append .CreateParameter("@dataIn",  	  		adVarChar, adParamInput,     8000, dataIn)
      .Parameters.Append .CreateParameter("@status",          adVarChar, adParamInput,     8000, vStatus)
      .Parameters.Append .CreateParameter("@custId",          adVarChar, adParamInput,      128, vEcom_CustId)
      .Parameters.Append .CreateParameter("@newAcctId",       adVarChar, adParamInput,      128, vEcom_NewAcctId)
      .Parameters.Append .CreateParameter("@id",              adVarChar, adParamInput,      128, vEcom_Id)
      .Parameters.Append .CreateParameter("@expires",         adVarChar, adParamInput,      128, vEcom_Expires)
    End With
    Set oRs = oCmdApp.Execute()
    If Not oRs.Eof Then fLogGuid = oRs("guid")
    Set oRs   = Nothing
	  Set oCmdApp = Nothing
	  sCloseDbApp
  End Function
  ...
  '... logs steps taken (apps/ecomSteps)
  Sub sStep (logNo, logStep)
    If b5Step Then
     	sOpenCmdApp
      With oCmdApp
        .CommandText = "sp5ecomSteps"
        .Parameters.Append .CreateParameter("@logNo",  	        adInteger, adParamInput,         , logNo)
        .Parameters.Append .CreateParameter("@logStep",  	  		adVarChar, adParamInput,      128, logStep)
      End With
      oCmdApp.Execute()
	    Set oCmdApp = Nothing
	    sCloseDbApp
  ...
  Function fPost
    fPost = ""  
    If Request.Body.Count = 0 Then fPost = "444 The service did not receive any values to process." : Exit Function

    For Each vFld In Request.Body
...
//    vValue = fUrlDecode(vValue) '...added Mar 1, 2016 to handle Bryan's WS which does a URLencode but uses forms - doesn't work with accents
      ...
      Select Case vFld
        ...
        Case "LogNo"                : logNo                = vValue   '...for loggging
    ...
    logNo       = fDefault(logNo, -1) '...if there's no logNo (simulator) set to -1
    dataIn      = Request.Body.Item '... this stores what's received

    sStep logNo, 0 '...if bStep is true then log this step
    ...
    If vEcom_Media = "AddOn2" Then
      If Len(vGroupCustId) > 0 and Len(vGroupId) > 0 Then
        ...
        If vMemb_Eof                   Then fPost = "471 You are trying to Add On to a Group site with a password that does not exist." : Exit Function   
        If vMemb_Level <> 3            Then fPost = "472 You are trying to Add On to a Group site with a password that is not assigned to a facilitator." : Exit Function   
...
'     vEcom_Expires              = fFormatSqlDate (Now + 90)  
    ...
    '...if Group2 (but NOT an AddOn2) create new customer, catalogue and add facilitator and Vubiz Internals plus the local support manager
    ElseIf Not bUpdateGroup2 Then
      ...
      If bCommit Then
        ...
        '...create the new customer record (vEcom_Id is not used in this call)
        ...
        If Len(vEcom_Id) = 0 Then
          ...
          vEcom_Id        = vMemb_Id
        Else
          vEcom_MembNo    = fNextMembNo2 (vEcom_NewAcctId, vEcom_Id)
        End If
      ...
      '...update the member table with the current member (facilitator)
      sStep logNo, 75
      ...
      '...add internals and default manager
      sStep logNo, 8
      If bCommit Then 
        sAddInternalMemb vMemb_AcctId
        vMemb_Internal = 0 : vMemb_No = 0 :  vMemb_Id = Left(vEcom_CustId, 4) & "_SALES" : vMemb_Level = 4 : vMemb_Manager = 1 : vMemb_Ecom = 1 : vMemb_Pwd = "BIGMGR" : sAddMemb vMemb_AcctId   '...add a client manager
      End If
