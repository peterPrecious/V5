...
<!--  <script>jTitle("/*--{[--*/Cardholder Information/*--]}--*/", 'Carhholder.jpg')</script>-->
  <img src="../Images/Ecom/CardholderInformation_<%=svLang %>.png" />

  <h1><!--[[-->Cardholder Information<!--]]--></h1>
  <h2><!--[[-->Specific cardholder information is required for all e-commerce transactions.&nbsp; The e-commerce fields must correspond to the billing information of the cardholder.&nbsp; This also determines what currency and taxes apply.&nbsp; Programs sold outside of Canada are in $US.&nbsp; Arrows at right show fields requiring input.<!--]]--></h2>
  <h3><!--[[-->Please click <b>Next</b> below to continue.<!--]]--></h3><br />
  ...
  <form method="POST" action="Ecom2Customer.asp" target="_self">
    <table class="table" id="customer">
      ...
      <tr>
        <th><!--[[-->Organization<!--]]--> : </th>
        <td <%=fIf(aErr(10), "class='notice'", "")%>>&nbsp;<input type="text" name="xxxCompany" value="<%=xxxCompany%>"> <!--[[-->Enter None if not applicable<!--]]--></td>
      </tr>
      <tr>
        <th><!--[[-->Address<!--]]--> : </th>
      ...
      <tr>
        ...
        <!--#include virtual = "V5/Inc/EcomProvince.asp"--></select> <!--[[-->Canada &amp; US only<!--]]--></td>
      </tr>
      <tr><th><!--[[-->Country<!--]]--> : </th>
      ...
      <tr>
        <td colspan="2" class="red" style="text-align:center; height:60px; vertical-align:middle;">
          ...
          <%   If svSecure Then %> 
          <!--[[-->You are signed into the profile of the Learner below and the courses you are currently purchasing are for this Learner’s profile.<br><br>To purchase a course for a Learner OTHER than the one named below, you must log in first as that Learner and then proceed to purchase.<!--]]-->
          <%   Else %> 
          <!--[[-->You MUST enter a Learner name below if it is different than the cardholder name above:<!--]]-->
          <%   End If %> 
          <% Else %> 
          <!--[[-->You MUST enter a Facilitator name below if it is different than the cardholder name above:<!--]]-->
          <% End If %>
        </td>
      </tr>
      ...
      <tr>
        <th>
          <% =fIf(vEcom_Media = "Online", "<!--{{-->Learner<!--}}-->", "<!--{{-->Facilitator<!--}}-->") %>&nbsp;<!--[[-->First Name<!--]]--> : 
        </th>
        <td>&nbsp;
          <% If svSecure Then %>
      ...
      <tr>
        <th>
          <% =fIf(vEcom_Media = "Online", "<!--{{-->Learner<!--}}-->", "<!--{{-->Facilitator<!--}}-->") %>&nbsp;<!--[[-->Last Name<!--]]--> : 
        </th>
        <td>&nbsp;
          <% If svSecure Then %>
            <% If svMembLevel = 5 Or svMembManager Then %>
      ...
      <tr>
        <th>
          <% =fIf(vEcom_Media = "Online", "<!--{{-->Learner<!--}}-->", "<!--{{-->Facilitator<!--}}-->") %>&nbsp;<!--[[-->Email Address<!--]]--> : 
        </th>
        <td>&nbsp;
          <% If svSecure Then %>
            <% If svMembLevel = 5 Or svMembManager Then %>
