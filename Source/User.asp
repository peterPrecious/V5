    ...
    <table class="table">
      <tr>
        <td colspan="2" style="text-align: center; height: 50px;">
          <h1><!--[[-->Add a Learner<!--]]-->/<!--[[-->Learner Profile<!--]]--></h1>
          <% 
          If vCust_MaxUsers > 0 And (vCust_Level < 3 Or svMembLevel = 5 Or svMembManager) Then 
            p0 = vCust_MaxUsers
            p1 = fAllMembCount - 1
          %>
          <h6 align="left"><!--[[-->Note: This account is limited to ^0 active or inactive learners.&nbsp; Once you reach the maximum you will be unable to add new learners or edit existing learners.&nbsp; You currently have ^1 learners on file.<!--]]--></h6>
          <% 
          End If 
      ...
      <tr>
        <th><%=fIf(svCustPwd or vCust_ChannelV8, "<!--{{-->Id<!--}}-->", "<!--{{-->Password<!--}}-->")%> :</th>
        <td>
          <% 
      ...
      <tr>
        ...
        <td>
          ...
          <!--[[-->Use English alpha, numeric and !@$%^*()_-{}[];<>,.: characters.&nbsp;Value is NOT case sensitive.<!--]]-->
      ...
      <tr>
        ...
        <td>
          ...
          %>
          <input type="radio" name="vMemb_Level" value="1" <%=fcheck(1, vmemb_level)%>>1 : <!--[[-->Guest<!--]]--> (<!--[[-->can access content and assessments (V8)<!--]]-->)<br>
          <% 
            End If 
          %>
...
-->
      ...
      <tr>
        ...
        %>
          <h5><br><br>
            <!--[[-->Learner Profiles accessed from another Account are Read Only.<!--]]-->
            <br>
          ...
          %> <%=f10%>
          <input onclick="jconfirm('User<%=fGroup%>.asp?vDelete=<%=vMemb_No%>&amp;vNext=<%=Server.UrlEncode(vNext)%>', '<!--[[-->Ok to delete?<!--]]-->')" type="button" value="<%=bDelete%>" name="bDelete" class="button085">
          <%=f5%> 
          <%   
              End If 
