...
<%
  ...
  Dim vCnt(5, 2)
  For i = 0 to 5 : vCnt (i, 0) = 0 : vCnt (i, 1) = 0 : vCnt (i, 2) = 0 : Next
  ...
  Do While Not oRs.Eof
    ...
    If i > 0 And i < 4 Then
      ...
      If vMemb_Sponsor > 0 Then vCnt(4, j) = vCnt(4, j) + 1
  ...
  vSql = "SELECT COUNT(DISTINCT Memb_1.Memb_No) AS Sponsors FROM Memb WITH (nolock) INNER JOIN Memb Memb_1 ON Memb.Memb_Sponsor = Memb_1.Memb_No WHERE (Memb.Memb_Active = 0) AND (Memb.Memb_AcctId = '" & svCustAcctId & "')"
  Set oRs = oDb.Execute(vSql)
  vCnt(5, 0) = Cint(oRs("Sponsors"))

  vSql = "SELECT COUNT(DISTINCT Memb_1.Memb_No) AS Sponsors FROM Memb WITH (nolock) INNER JOIN Memb Memb_1 ON Memb.Memb_Sponsor = Memb_1.Memb_No WHERE (Memb.Memb_Active = 1) AND (Memb.Memb_AcctId = '" & svCustAcctId & "')"
  Set oRs = oDb.Execute(vSql)
  vCnt(5, 1) = Cint(oRs("Sponsors"))
...
<body>
  ...
  <form method="POST" action="UsersOnsite.asp">
    <table border="0" width="100%" bordercolor="#DDEEF9" style="border-collapse: collapse" cellpadding="0" cellspacing="0">
      <tr>
        <td><h1 align="center">The Learners Onsite Report </h1><h2>This displays who was Active and Inactive during a particular period over the past 12 months or overall.&nbsp; After you have made your selections, click <b>Go</b>.<br><br>If you choose to count all learners, then learners are considered Inactive if they are currently flagged as inactive, otherwise they are considered Active.&nbsp; However: If you select start and end dates, then learners are considered Inactive if their last site access date was between the dates selected and they are currently flagged as Inactive.&nbsp; Learners are considered Active if their first site access date was between the selected dates and they are currently flagged as Active.&nbsp; Learners that were neither active nor inactive during the time selected, but are currently on file, will be considered as Other.</h2></td>
      </tr>
      <tr>
        <td>
      ...
      <tr>
        <td>&nbsp;<div align="center"><center>
          <table border="1" style="border-collapse: collapse" bordercolor="#DDEEF9" id="AutoNumber2" cellspacing="0" cellpadding="5">
            <tr>
              <th width="20%" nowrap>&nbsp;</th>
              <th align="right" width="20%" nowrap>Active</th>
              <th align="right" width="20%" nowrap>Inactive</th>
              <th align="right" width="20%" nowrap>Other</th>
              <th align="right" width="20%" nowrap>Total</th>
            </tr>

<% If vCust_MaxSponsor > 0 Then %>

            <tr>
              <th width="20%" align="right" nowrap>Sponsored Learners</th>
              <td align="right" width="20%"><%=vCnt(4, 1)%> </td>
              <td align="right" width="20%"><%=vCnt(4, 0)%> </td>
              <td align="right" width="20%"> 0</td>
              <td align="right" width="20%"><%=vCnt(4, 1)+ vCnt(4, 0)%> </td>
            </tr>
            <tr>
              <th width="20%" align="right" nowrap>Sponsors</th>
              <td align="right" width="20%"><%=vCnt(5, 1)%> </td>
              <td align="right" width="20%"><%=vCnt(5, 0)%> </td>
              <td align="right" width="20%"> 0</td>
              <td align="right" width="20%"><%=vCnt(5, 1)+ vCnt(5, 0)%> </td>
            </tr>
            <tr>
              <th width="100%" align="right" nowrap colspan="5">&nbsp;</th>
            </tr>
