...
%>
  <table border="0" width="100%" cellspacing="0" cellpadding="0">
    <tr>
      ...
      %>
      <!--webbot bot='PurpleText' PREVIEW='Thank you. Please click <b>Next</b> to continue with Bank'--><%=fPhra(000293)%>&nbsp<%=Session(vModID & "Bank")+1%>...<br><br>
      <!--webbot bot='PurpleText' PREVIEW='or, you can take a break and leave the site. <br>When you return you will be positioned at the next bank.'--><%=fPhra(000207)%></p></td>
    </tr>
    ...
    <tr>
      ...
      <% ElseIf (CInt(Session(vModID & "Bank")) * 5) >= CInt(Session(vModID & "MinQue")) AND vTotal < (CInt(Session(vModID & "PassGrade"))/100) And (CInt(Session(vModID & "Attempt")) < CInt(Session(vModID & "MaxAttempts"))) Then 
    		...
    		Session(vModID & "TestStarted") = False
      %>
      <!--webbot bot='PurpleText' PREVIEW='You have completed the exam with a score below'--><%=fPhra(000294)%>&nbsp;<%=Session(vModID & "PassGrade")%>%.
      <br><!--webbot bot='PurpleText' PREVIEW='You are able to take this Examination again.'--><%=fPhra(000295)%><br>
      ...
      %>
      <!--webbot bot='PurpleText' PREVIEW='You have completed the exam with a score below'--><%=fPhra(000294)%>&nbsp;<%=Session(vModID & "PassGrade")%>%.
      <br><!--webbot bot='PurpleText' PREVIEW='You DO NOT have any more attempts at this Examination left.'--><%=fPhra(000296)%><br>
      <p align="center"><a href="javascript:window.close()"><img border="0" src="../Images/Buttons/Close_<%=svLang%>.gif"></a></p>
      ...
      %>
      <!--webbot bot='PurpleText' PREVIEW='You have completed the exam successfully with a passing grade.'--><%=fPhra(000040)%><br>
      <% ElseIf (CInt(Session(vModID & "Bank")) * 5) >= CInt(Session(vModID & "MinQue")) Then
    		Session(vModID & "ExamPassed") = True
    		Session(vModID & "TestStarted") = False
    		Response.Redirect "ExamGrade.asp?vModID=" & vModID & "&vTotalGrade=" & vTotal
      %>
      <!--webbot bot='PurpleText' PREVIEW='You have completed the exam successfully with a passing grade; you may now claim a Certificate.'--><%=fPhra(000041)%><br>
      <% End If %> 
