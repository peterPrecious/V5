...
<%
  ...
  If Len(vList) = 0 Then 
    Response.Redirect "UsersBulkInput.asp?vErrMess=" & Server.URLEncode("Please enter data to Import into the Text Area.") & vReturnInfo
  '...Check if delimiters are the same for Field and Record
  ElseIf vFieldSep = vRecordSep Then
    Response.Redirect "UsersBulkInput.asp?vErrMess=" & Server.URLEncode("Please ensure that the Field Delimeter and Record Delimeter are <i>not</i> the same.") & vReturnInfo
  End If
  ...
  For i = 0 to vNumRecords
    ...
    If Ubound(aTemp) <> (vFieldCount - 1) Then
      Response.Redirect "UsersBulkInput.asp?vErrMess=" & Server.URLEncode("Invalid number of Fields in Record " & i+1 & ".") & vReturnInfo
    '...Check if Password is blank
    ElseIf Len(aTemp(0)) = 0 Then
      Response.Redirect "UsersBulkInput.asp?vErrMess=" & Server.URLEncode("Password is blank in Record " & i+1 & ".") & vReturnInfo
    End If
  ...
  sOpenDb
  vSql = "SELECT Memb_Id, Memb_Email FROM Memb WITH (nolock) WHERE Memb_AcctId = '" & svCustAcctId & "'"
  ...
  For i = 0 to vNumRecords
    ...
    '...Check if UserId are Valid
    aTemp(0) = Trim(Ucase(aTemp(0)))
    If fMembIdOk(aTemp(0)) = False Then
      vErrMess = vErrMess & "Learner Id [" & aTemp(0) & "]" & " in record " & i+1 & " is not valid.<br>"
		End If
...
'       If Not fIsEmail(Trim(Ucase(aTemp(vEmailPos-1)))) Then
'         vErrMess = vErrMess & "Email [" & aTemp(vEmailPos-1) & "]" & " is not valid.<br>"
'       End If

        '...Check if Email is already on list just uploaded 
  ...
  If Len(vErrMess) > 0 Then
    Response.Redirect "UsersBulkInput.asp?vErrMess=" & Server.URLEncode(fLeft(vErrMess, 1000)) & vReturnInfo
  End If
...
<head>
  <title>UserBulkInputVerify</title>
  <meta charset="UTF-8">
...
<body>
  ...
  <h1>Upload Learner Profiles (Basic)&nbsp; - Verify</h1>
  <h2>Please verify the following information is ok to Upload...</h2>
  ...
  <table class="table">
      ...
      For i = 0 to Ubound(aAllRecords, 2)
        If i = 0 Then
          ...
          For k = 0 to Ubound(aAllRecords, 1)
            Response.Write "<th class='rowshade' width='" & vRecWidth & "%' style='text-align:left' bgcolor='#DDEEF9'>" & aFieldNames(k) & "</th>"
          Next
          Response.Write "</tr>"
        End If
    ...
    %>
    <tr>
      <td style="text-align:center" colspan="<%=vFieldCount %>">&nbsp;<p>
        <input onclick="location.href = 'UsersBulkInput.asp?abc=<%=vReturnInfo%>'" type="button" value="Edit Values" name="bEdit" class="button"><%=f10%>
        <input onclick="location.href = 'UsersBulkInputOK.asp?abc=<%=vReturnInfo%>'" type="button" value="Save Values" name="bSave" class="button">
      </p>
        <h2><a href="Users.asp">Learner List</a></h2>
      </td>
    </tr>
  </table>
