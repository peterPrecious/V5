	...
	<table class="table">
		<tr>
			<td style="white-space:nowrap">
				<script>jTitle("<%=fPhraH(000182)%>", 'SingleLicense.jpg')</script>
			</td>
			<td style="white-space:nowrap">
				<h1><!--webbot bot='PurpleText' PREVIEW='All available courses are listed by category below. &nbsp;To navigate:'--><%=fPhra(001277)%></h1>
				<ol class="c2">
					<li><!--webbot bot='PurpleText' PREVIEW='Click on a bolded <b>Program Title</b> to expand the program.'--><%=fPhra(001274)%></li>
					<li><!--webbot bot='PurpleText' PREVIEW='Click on a black <font color="#000000">Module Title</font> to launch a module.'--><%=fPhra(001275)%></li>
					<li><!--webbot bot='PurpleText' PREVIEW='Click on a green <font color="#008000">[Status]</font> link for details of your learning activities.'--><%=fPhra(001276)%></li>
					<li><!--webbot bot='PurpleText' PREVIEW='Sort by'--><%=fPhra(000243)%>: <a href="MyContent.asp?vSort=n"><!--webbot bot='PurpleText' PREVIEW='Default Order'--><%=fPhra(000310)%></a> | <a href="MyContent.asp?vSort=y"><!--webbot bot='PurpleText' PREVIEW='Category Order'--><%=fPhra(000311)%></a>.</li>
				</ol>
			</td>
			<td style="text-align:center; white-space:nowrap">
				<% If svMembLevel = 5 Or svMembManager Then %><a class="c2" href="Patience.asp?vNext=RTE_MyContent.asp"><!--webbot bot='PurpleText' PREVIEW='New view!'--><%=fPhra(001398)%></a><% End If %>
			</td>
		</tr>
	</table>
...
%>
							...
							<tr>
								...
								<table border="0" width="100%" id="table14" cellspacing="0" cellpadding="2">
									...
									<tr>
										...
										<!--webbot bot='PurpleText' PREVIEW='Expires'--><%=fPhra(000137)%>:</b> <%=fFormatDate(DateAdd("d", -1, vExpires))%><br></p></td>
									</tr>
									<% End If %> 
									...
									<tr>
										<td width="5%">&nbsp;</td>
										<td width="90%"><br><!--webbot bot='PurpleText' PREVIEW='Estimated program length'--><%=fPhra(000130)%> : <%=vProg_Length%>&nbsp;<!--webbot bot='PurpleText' PREVIEW='Hour(s)'--><%=fPhra(000143)%>.<br>&nbsp;</td>
									</tr>
									<tr>
										<td width="5%">&nbsp;</td>
										<td width="90%"><p class="c2"><b>
										<font color="#000000"><!--webbot bot='PurpleText' PREVIEW='Modules'--><%=fPhra(000179)%></font> :</b></p></td>
									</tr>
									<% End If %>
										...
										<tr>
											...
											<td <%=vbg%> nowrap class="c8">
												...
												<% If Len(Trim(vMods_AssessmentUrl)) > 0 Then %>
													[<a <class="c8" <%=fstatx%> href="javascript:<%=vMods_AssessmentScript%>('<%=vMods_AssessmentUrl%>','<%=vMods_Id%>')"><!--webbot bot='PurpleText' PREVIEW='Assessment'--><%=fPhra(000073)%></a>] 
												<% End If %> 
											</td>
											<td <%=vbg%> nowrap width="30%" class="green">
												[<a href="javascript:SiteWindow('ModuleDescription.asp?vClose=Y&vModId=<%=vMods_Id%>')"><!--webbot bot='PurpleText' PREVIEW='Description'--><%=fPhra(000118)%></a>]
	...
	'                              End If
											...
											%> 
														<a <%=fstatx%> class="c8" href="javascript:fullScreen('<%=fCertificateUrl("", "", vBestScore, vLastScore, vProg_Assessment, vTitle, "", "", "", vProg_Id, "", "", "")%>')"><!--webbot bot='PurpleText' PREVIEW='Examination'--><%=fPhra(000132)%></a>
											<% 
												Else
											...
											%> 
														<% If vMods_FullScreen Then %>
														<a <%=fstatx%> href="javascript:fullScreen('<%=vProg_Id%>|<%=vMods_Id%>|<%=vProg_Test%>|<%=vProg_Bookmark%>|<%=vProg_CompletedButton%>')"><!--webbot bot='PurpleText' PREVIEW='Examination'--><%=fPhra(000132)%></a> 
														<% ElseIf Ucase(vMods_Type) = "FX" Then %>
														<a class="c8" <%=fstatx%> href="/V5/LaunchObjects.asp?vModId=<%=vProg_Id%>|<%=vMods_Id%>&vNext=<%=vNext%>"><!--webbot bot='PurpleText' PREVIEW='Examination'--><%=fPhra(000132)%></a>
														<% Else %>
														<a class="c8" <%=fstatx%> href="javascript:<%=vMods_Script%>('<%=vProg_Id%>|<%=vMods_Id%>|<%=vProg_Test%>|<%=vProg_Bookmark%>|<%=vProg_CompletedButton%>')"><!--webbot bot='PurpleText' PREVIEW='Examination'--><%=fPhra(000132)%></a> 
														<% End If %>
											<% 
													Else 
											%> 
															<a <%=fstatx%> href="javascript:fAlert()"><!--webbot bot='PurpleText' PREVIEW='Examination'--><%=fPhra(000132)%></a> 
											<% 
													End If 
	...
	'                                End If
												...
												If vOk Then   													
											%> 
													<a <%=fstatx%> class="c8" href="javascript:fullScreen('<%=fCertificateUrl("", "", vBestScore, vLastScore, vProg_Assessment, vTitle, svLang, "", "", vProg_Id, "", "", "")%>')"><!--webbot bot='PurpleText' PREVIEW='Examination'--><%=fPhra(000132)%></a>
											<%
	...
	<!-- don't use vProg_Cert any more -->      
										...
										<tr>
											<td width="5%">&nbsp;</td>
											<td <%=vbg%> class="c8">
												<a <%=fstatx%> href="javascript:vuwindow('CertCompletion.asp?vProgId=<%=vProg_Id%>&vClose=Y',650,425,100,100,'no','no','no')"><!--webbot bot='PurpleText' PREVIEW='Certificate of Completion'--><%=fPhra(000095)%></a>
											</td>
											<td <%=vbg%> class="c2">&nbsp;</td>
										</tr>
										<% End If %>
	...
	%>                              
													<a <%=fstatx%> class="c8" href="javascript:fullScreen('<%=fCertificateUrl("", "", vBestScore, i, vProg_Assessment, vTitle, "", "", "", vProg_Id, "", "", "")%>')"><!--webbot bot='PurpleText' PREVIEW='Certificate of Completion'--><%=fPhra(000095)%></a>
	<%
												Else
													vUrl = fPhraH(000467) & "&nbsp;" & FormatPercent(vProg_AssessmentScore, 0) & "&nbsp;" & fPhraH(000468)
													vUrl = "Error.asp?vErr=" & Server.UrlEncode(vUrl) & "&vClose=Y&vReturn=close"
	%>
												<a class="black" <%=fstatx%> href="#" onclick="window.open('<%=vUrl%>','Vubiz','toolbar=no,scrollbars=yes,width=750,height=500')"><!--webbot bot='PurpleText' PREVIEW='Certificate of Completion'--><%=fPhra(000095)%></a>
	<%
												End If
	%>
											</td>
