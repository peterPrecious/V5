<%
	...
	'...fMyWorld creates the html for MyWorld tasklist using "vContent"
	Function fMyWorld (vAcctId, vId, vActionX)
	...
	Sub sLevel_0
		...
		vContent = vContent & "  <td colspan='4' width='600'><div class='c1' style='text-align:center'>" & vTskH_Title & "</div><h2>" & vTskH_Desc & vPassword & "</div></h2></td>" & vbCrLf
		vContent = vContent & "</tr>" & vbCrLf
	End Sub

	Sub sLevel_1
		vContent = vContent & "<tr>" & vbCrLf
		vContent = vContent & "  <td align='center' style='width:25px; text-align:center; vertical-align:middle'>" & fAccess & " </td>" & vbCrLf
		vContent = vContent & "  <td colspan='3' width='575'><div class='c2'>" & vTskH_Title & "</div><div>" & vTskH_Desc & vPassword & "</div></td>" & vbCrLf
		vContent = vContent & "</tr>" & vbCrLf
	End Sub
	...
	Sub sLevel_2
		Dim vOk
		vContent = vContent & "<tr>" & vbCrLf
		vContent = vContent & "  <td style='width:25px; text-align:center; vertical-align:middle'>&nbsp;</td>" & vbCrLf
		vContent = vContent & "  <td style='width:25px; text-align:center; vertical-align:middle'>" & fAccess & " </td>" & vbCrLf
		If Len(Trim(vTskH_Title)) > 0 Then
		vContent = vContent & "  <td colspan='2' style='width:550px;'><div class='c3'>" & vTskH_Title & "</div><div>" & vTskH_Desc & vPassword & "</div></td>" & vbCrLf
		Else
		vContent = vContent & "  <td colspan='2' style='width:550px;'>" & vTskH_Desc & vPassword & "</td>" & vbCrLf
		End If
		vContent = vContent & "</tr>" & vbCrLf
...
'   If vOk And Not fIsLocked (Left(svCustId, 4), svMembNo) Then vOk = True
					...
					Select Case vTskD_Type
						...
						Case "M", "MR", "MT", "AM", "10"  '...Module Live/Review/Test Prereq/Accessible/VuAssess
							...
							If vTskD_Type = "M" Or  vTskD_Type = "MT" Or vTskD_Type = "AM" Then
								vAlt   = Server.HtmlEncode(fPhraH(000168))
							ElseIf vTskD_Type = "10" Then  
								vAlt   = Server.HtmlEncode(fPhraH(000073))
							Else  
								vAlt   = Server.HtmlEncode(fPhraH(000178))
							End If
...
'                 sGetMods (Mid(vTskD_Id, 9, 6))  '...this does not handle big mod nos
									vMods_Id = Mid(vTskD_Id, 9)     '...might include pipes
									If Instr(vMods_Id, "|") > 0 Then vMods_Id = Left(vMods_Id, Instr(vMods_Id, "|") - 1)
									sGetMods (vMods_Id)
...
'             ElseIf (Ucase(vMods_Type) = "FX" Or Ucase(vMods_Type) = "XX" Or Ucase(vMods_Type) = "Z") And vMods_FullScreen Then
							...
							If vPreReq Then
								vUrlTitle1 = Server.HtmlEncode(fPhraH(000828))
							...
							'...remaining assets
							Else
								vUrlTitle1 = Server.HtmlEncode(fPhraH(000829))
								vTitle = vTitle & " [<a " & fStatX & " href=""javascript:SiteWindow('ModuleDescription.asp?vClose=Y&vModId=" & vMods_Id & "')"" title='" & vUrlTitle1 & "'>" & fPhraH(000118) & "</a>]"
'               vTitle = vTitle & "<br><span class='green'>[" & fModStatusLink (svMembNo, vProg_Id, vMods_Id) & "]</span>"
...
'             vStatusTitle = vTitle
							...
							vUrlTitle1 = Server.HtmlEncode(fPhraH(000828))
							vUrlTitle2 = Server.HtmlEncode(fPhraH(000829))
							vTitle = "<a " & fStatX & " href=" & Chr(34) & vUrl & Chr(34) & " title='" & vUrlTitle1 & "'>" & vTitle & "</a> "
							vTitle = vTitle & "[<a " & fStatX & " href=""javascript:SiteWindow('ModuleDescription.asp?vClose=Y&vModId=" & vMods_Id & "')"" title='" & vUrlTitle2 & "'>" & fPhraH(000118) & "</a>]"
...
'             vTitle = vTitle & "<br><span class='green'>[" & fModStatusLink (svMembNo, vProg_Id, vMods_Id) & "]</span>"
						...
						Case "P","PC","U","UC","O","OC","R","RC","J","JC"  
							...
							vIcon  = "offline.gif"
							vAlt   = Server.HtmlEncode(fPhraH(000297))
						...
						Case "PE"
							vIcon  = "offline.gif"
							vAlt   = Server.HtmlEncode(fPhraH(000885))
						...
						Case "L" '...Link
							vIcon = "form.gif"
							vAlt   = Server.HtmlEncode(fPhraH(000222))
						...
						Case "V", "D", "C"  '...Documents: Private, Client, Common
							vIcon  = "document.gif"
							vAlt   = Server.HtmlEncode(fPhraH(000224))
							'...if there's some parms add "&" else add "?"
							If Instr(vTskD_Id, "?") > 0 Then
								vTskD_Id = vTskD_Id & "&"
							Else
...
'             vTitle = vTskD_Title
						...
						Case "T" '...Test
							vIcon = "checkmark.gif"
							vAlt   = Server.HtmlEncode(fPhraH(000235))
							vUrl = vTskD_Id
							vTitle = vTskD_Title
							sLevel_3
						...
						Case "E", "ET" '...Exam and Exam with Prerequisite
							...
							vAlt   = Server.HtmlEncode(fPhraH(000132))
						...
						Case "Y" '...Title
							vIcon  = "line.gif"
							vAlt   = Server.HtmlEncode(fPhraH(000019))
							vUrl   = ""
							vTitle = vTskD_Title
							sLevel_3
	...
	Sub sLevel_3
		...
		If vIcon = "line.gif" Then '...title, bold
			vContent = vContent & "  <td style='width:25px; text-align:center; vertical-align:middle'><img border='0' src='../Images/Icons/" & vIcon & "' alt='" & vAlt & "'></td>" & vbCrLf
			vContent = vContent & "  <td style='width:550px;'>" & vTitle & "</td>" & vbCrLf
		ElseIf vIcon = "lock.gif" Then '...title, bold
			vContent = vContent & "  <td style='width:25px; text-align:center; vertical-align:middle'><img border='0' src='../Images/Icons/" & vIcon & "' alt='" & vAlt & "'></td>" & vbCrLf
			vContent = vContent & "  <td style='width:550px;'>" & vTitle & "</td>" & vbCrLf
		ElseIf vAction <> "template" And vUrl <> "" Then
			vContent = vContent & "  <td style='width:25px; text-align:center; vertical-align:middle'><a " & fStatX & " href=" & vUrl & vTarget & "><img border='0' src='../Images/Icons/" & vIcon & "' alt='" & vAlt & "'></a></td>" & vbCrLf
			vContent = vContent & "  <td style='width:550px;'>" & vTitle & "</td>" & vbCrLf
		Else
			vContent = vContent & "  <td style='width:25px; text-align:center; vertical-align:middle'><img border='0' src='../Images/Icons/" & vIcon & "' alt='" & vAlt & "'></td>" & vbCrLf
			vContent = vContent & "  <td style='width:550px;'>" & vTitle & "</td>" & vbCrLf
	...
	Sub sLevel_99
		...
		vContent = vContent & "  <td style='width:25px; text-align:center; vertical-align:middle'> </td>" & vbCrLf
		vContent = vContent & "  <td style='width:25px; text-align:center; vertical-align:middle'> </td>" & vbCrLf
		vContent = vContent & "  <td style='width:550px;'> </td>" & vbCrLf
		vContent = vContent & "</tr>" & vbCrLf
	End Sub
	...
	Function fAccess
		...
		If vAction <> "template" Then
			If vTskH_Locked And fIsLocked (vTskH_No, svMembNo) Then
				fAccess = "<img border='0' src='../Images/Icons/lock.gif' alt='" & Server.HtmlEncode(fPhraH(000172)) & "' width='18' height='22'>"
				vLocked = True
			ElseIf Not vTskH_Child Then
				fAccess = "&nbsp;"
			ElseIf vAccess = 0 Then
'...anchor bug in XP SP2 - removed
'       fAccess = "<a name='" &  vTskH_Order & "' href='MyWorldRedirect.asp?vTskH_AcctId=" & vTskH_AcctId & "&vTskH_Id=" & vTskH_Id & "&vToggle=" & vTskH_Order & "#" &  vTskH_Order & "'><img border='0' src='../Images/Common/VuPlus.gif'  alt='" & Server.HtmlEncode(fPhraH(000136)) & "'></a>"
				fAccess = "<a name='" &  vTskH_Order & "' href='MyWorldRedirect.asp?vTskH_AcctId=" & vTskH_AcctId & "&vTskH_Id=" & vTskH_Id & "&vToggle=" & vTskH_Order & "'><img border='0' src='../Images/Common/VuPlus.gif'  alt='" & Server.HtmlEncode(fPhraH(000136)) & "'></a>"
			Else
'...anchor bug in XP SP2 - removed
'       fAccess = "<a name='" &  vTskH_Order & "' href='MyWorldRedirect.asp?vTskH_AcctId=" & vTskH_AcctId & "&vTskH_Id=" & vTskH_Id & "&vToggle=" & vTskH_Order & "#" &  vTskH_Order & "'><img border='0' src='../Images/Common/VuMinus.gif' alt='" & Server.HtmlEncode(fPhraH(000106)) & "'></a>"
				fAccess = "<a name='" &  vTskH_Order & "' href='MyWorldRedirect.asp?vTskH_AcctId=" & vTskH_AcctId & "&vTskH_Id=" & vTskH_Id & "&vToggle=" & vTskH_Order & "'><img border='0' src='../Images/Common/VuMinus.gif' alt='" & Server.HtmlEncode(fPhraH(000106)) & "'></a>"
			End If
		End If
	End Function
	...
	Sub sCreateLink (vProgId)
		...
		For k = 0 to Ubound(aMods)
			...
			'...if the module table contains height/width values use them, else use the script 
'     If vMods_Width > 0 And vMods_Height > 0 Then 
'       vUrl   = "javascript:launchObjects ('" & vUrl & "', " & vMods_Width & ", " & vMods_Height & ")"
'     Else
...
'     ElseIf (Ucase(vMods_Type) = "FX" Or Ucase(vMods_Type) = "XX") And vMods_FullScreen Then
			...
			vTitle = vTitle & "      <td style='width:550px;'><p><a " & fStatX & " href=""" & vUrl & """ title='" & vUrlTitle1 & "'>" & vMods_Title & "</a>"
			If fModsDesc (vMods_Id) Then
				vTitle = vTitle & " [<a " & fStatX & " href=""javascript:SiteWindow('ModuleDescription.asp?vClose=Y&vModId=" & vMods_Id & "')"" title='" & vUrlTitle1 & "'>" & fPhraH(000118) & "</a>]"
			End If
'     vTitle = vTitle & "<br><span class='green'>[" & fModStatusLink (svMembNo, vProg_Id, vMods_Id) & "]</span></td>" & vbCrLf
		...
		If Len(vProg_Assessment) > 0 Then  
			...
			vTitle = vTitle & "    <tr>" & vbCrLf
			vTitle = vTitle & "      <td style='width:550px;'>"
			vTitle = vTitle &          fVuAssessLink (vMods_Id, fPhraH(000132), vMods_Title)
			vTitle = vTitle & "      </td>"
			vTitle = vTitle & "    </tr>" & vbCrLf
		...
		ElseIf Lcase(vProg_Exam) <> "n" Then  
			...
			vUrlTitle1 = Server.HtmlEncode(fPhraH(000830))
			vTitle = vTitle & "    <tr>" & vbCrLf
			vTitle = vTitle & "      <td style='width:550px;'><a " & fStatX & " href=""javascript:examwindow('" & vProg_Exam & "')"" title='" & vUrlTitle1 & "'>Examination</a>"
			vExamId = Mid(vProg_Exam, 22, 6)
			If fExamOk(vExamId) Then
'     vTitle = vTitle & "        <br><span class='green'>[" & fAssessmentStatus (svMembNo, vExamId) & "]</span>"
