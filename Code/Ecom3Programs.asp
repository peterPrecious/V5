...
<!--              <script>jTitle("<%=fPhraH(000377)%>", 'MultiLicense.jpg')</script>-->
        ...
        <h1>
          <!--webbot bot='PurpleText' PREVIEW='Programs'--><%=fPhra(000203)%></h1>

        <p class="c3" style="text-align: left; padding-bottom: 10px;">
          <!--webbot bot='PurpleText' PREVIEW='To purchase a program, click the <b>Add</b> button and the program will be added to your basket and removed from this list.&nbsp; Click <b>Return</b> to come back to this list to add more programs.'--><%=fPhra(000318)%>
          <!--webbot bot='PurpleText' PREVIEW='Click on the program title to view the program/module description.'--><%=fPhra(000319)%>&nbsp;&nbsp;
          <!--webbot bot='PurpleText' PREVIEW='Note: The prices below are individual seat prices.&nbsp; Multi-learner discounts are applied in the next step when the number of seats is entered.'--><%=fPhra(000382)%>
          <!--webbot bot='PurpleText' PREVIEW='<b>Terms of License</b>: Your license is active for one year (365 days) from the date of purchase.'--><%=fPhra(000961)%>
          <!--webbot bot='PurpleText' PREVIEW='As the Facilitator of this Web based service, you can take the purchased course(s) yourself at no additional charge.'--><%=fPhra(001215)%>
        </p>

      </td>
    </tr>
    <tr>
      <td style="text-align: center;"><a href="javascript:toggle('div_discounts');" class="c3"><!--webbot bot='PurpleText' PREVIEW='Show Multi-learner Discounts'--><%=fPhra(000922)%></a>&ensp;&ensp;
      <div id="div_discounts" class="div">
        <table class="table">
          <tr>
            <td style="text-align: center; height: 30px; background-color: #DDEEF9"><!--webbot bot='PurpleText' PREVIEW='The following multi-learner discounts apply.'--><%=fPhra(000381)%></td>
          </tr>
          <tr>
            <td style="text-align: center">
              <table style="width: 400px; margin: auto;">
                <tr>
                  <th style="text-align: center" colspan="3"><!--webbot bot='PurpleText' PREVIEW='Total Seats'--><%=fPhra(000282)%></th>
                  <th style="text-align: center"><!--webbot bot='PurpleText' PREVIEW='Discount'--><%=fPhra(000120)%></th>
                </tr>
                <tr>
              ...
              <% End If %>
              <p>
                <a href="mailto:<%=fDefault(vCust_Email, "info@vubiz.com")%>?subject=Multi User License Enquiry"><b><font color="#FF0000"><!--webbot bot='PurpleText' PREVIEW='Email us'--><%=fPhra(001220)%></font></b></a><font color="#FF0000">&ensp;<!--webbot bot='PurpleText' PREVIEW='if you are interested in more seats.'--><%=fPhra(001603)%></font>
            </td>
          </tr>
        </table>
        <br>
      </div>
        <a <%=fstatx%> href="Ecom3Basket.asp?vPage=Ecom3Programs.asp" class="c3"><!--webbot bot='PurpleText' PREVIEW='Show My Basket'--><%=fPhra(000239)%></a><br>&nbsp;
      </td>
    </tr>
  </table>
...
'       If vProg_US > 0 And vProg_US <> 9999 And vProg_CA <> 9999 And Instr(vEcom_Programs, vProg_Id) = 0 And Instr(vMemb_Programs, vProg_Id) = 0 Then
    ...
    <tr>
      <th class="rowshade" style="width: 50%; text-align: left;"><!--webbot bot='PurpleText' PREVIEW='Program Title'--><%=fPhra(000320)%></th>
      <th class="rowshade" colspan="2" style="width: 20%;">
        <table class="table">
          <tr>
            <th class="rowshade" style="text-align: right;" colspan="2"><!--webbot bot='PurpleText' PREVIEW='Per Seat Cost'--><%=fPhra(000212)%><b>*</b></th>
          </tr>
          <tr>
      ...
      <th class="rowshade" style="width: 20%; text-align: center;"><!--webbot bot='PurpleText' PREVIEW='Add to Basket'--><%=fPhra(000321)%></th>
    </tr>
  ...
  <% If vCnt = 0 Then %>
  <h6>
    <!--webbot bot='PurpleText' PREVIEW='There are no programs available for purchase in the category'--><%=fPhra(000002)%>
    <b><%=vCatl_Title%></b>.&nbsp;
    <!--webbot bot='PurpleText' PREVIEW='Previously purchased programs can be accessed by clicking on the &quot;My Content&quot; tab above.&nbsp; If you have just selected them, they can be accessed by clicking on <b>Show My Basket</b> link.'--><%=fPhra(000329)%>
  </h6>
  <% Else %>
  <h3>
    <!--webbot bot='PurpleText' PREVIEW='Click on the program title to view the program/module description.'--><%=fPhra(000319)%></h3>
  <h3><b>* </b>
    <!--webbot bot='PurpleText' PREVIEW='Applicable taxes extra for Canadian orders.&nbsp;
    <br>
    Products purchases outside Canada are payable in US funds.'--><%=fPhra(001783)%>
  </h3>
  <% End If %>
