...
<%
  Dim vCnt, vTaxType, vTaxRate, vLearners, vCurLearners, vCustExpires, vPrice, vAmount, vTax, vTotal, vTotAmount, vTotTax, vTotTotal, vEcomNo, vDate, vUrl, vTotLearners
  ...
  vCustExpires = fCustExpires(Left(vEcom_CustId, 4) & vEcom_NewAcctId)
...
//--></script><!--webbot BOT="GeneratedScript" endspan --><form name="FrontPage_Form1" method="POST" action="EcomAdjustG2.asp" target="_self" onsubmit="return FrontPage_Form1_Validator(this)" language="JavaScript">
    ...
    <table border="0" width="100%" cellpadding="10" bordercolor="#DDEEF9" style="border-collapse: collapse">
      <tr>
        ...
        <h1 align="center">Adjust Ecommerce Transactions for Group 2 Sales</h1><h2>This allows you increase (or decrease) the number of seats in a Group 2 Site by adding an <b>Adjustment</b> record for each program originally ordered.&nbsp; <span style="background-color: #FFFF00">Enter fields highlighted in yellow</span>&nbsp;then click <b>Calculate</b> to confirm total cost of the transaction. When ready click <b>Update</b> making the adjustments permanent.</h2>
        <% Else %>
        <h1 align="center">Ecommerce Transactions for Group 2 Sales</h1><h2>This shows the original ecommerce transactions and any subsequent adjustments.</h2>
        <% End If %>
        </td>
      </tr>
      <tr>
...
<%
  ...
  Do While Not oRs.Eof
    ...
    If vCnt = 0 Then
      ...
      '...determine tax
      vTaxRate = fGST(vEcom_Country, vEcom_Province) 
      If vTaxRate > 0 Then 
        vTaxType = "GST" 
      Else  
        vTaxRate = fHST(vEcom_Country, vEcom_Province) 
        If vTaxRate > 0 Then 
          vTaxType = "HST" 
        End If
      End If
%>
      ...
      <tr>
        ...
        <table border="1" id="table6" cellspacing="0" cellpadding="2" style="border-collapse: collapse" bordercolor="#DDEEF9" width="600">
          <tr>
            <th bgcolor="#DDEEF9" bordercolor="#FFFFFF" colspan="8" height="30">Transaction History</th>
          </tr>
          <tr>
...
<% If svMembLevel = 5 Then %>
          ...
          <tr>
            <th bordercolor="#FFFFFF" height="30" colspan="8">&nbsp;
              <table border="1" id="table7" cellspacing="0" cellpadding="4" bordercolor="#DDEEF9" style="border-collapse: collapse">
                <tr>
                  ...
                  <th align="right" bgcolor="#FFFF00">&nbsp;<!--webbot bot="Validation" s-display-name="New Learners" s-data-type="Integer" s-number-separators="x" b-value-required="TRUE" i-minimum-length="1" i-maximum-length="3" s-validation-constraint="Greater than or equal to" s-validation-value="1" s-validation-constraint="Less than or equal to" s-validation-value="999" --><input type="text" name="vLearners" size="2" maxlength="3" style="text-align: right" value="<%=vLearners%>">&nbsp;</th>
                </tr>
              </table>&nbsp;
            </th>
          </tr>
...
%>
            <tr>
              <th colspan="5" bgcolor="#DDEEF9" bordercolor="#FFFFFF" height="30" align="right">Total New Adjustments $&nbsp;&nbsp;&nbsp; </th>
...
<% If svMembLevel = 5 Then %>
        ...
        <h2>Before you <b>Update</b>, generate Totals by clicking <b>Calculate</b>.</h2>
        <p><input type="submit" value="Calculate" name="bCalculate" class="button"></p>
        <h2>If the above values are correct <br>(ie you have not changed the <b># of Learners to Add</b> nor the <b>Price Each</b> since you clicked <b>Calculate</b>)<br>then click <b>Update</b> and your <b>New</b> transactions will become permanent <b>Adj</b>ustments above in green.</h2>
        <p><input type="submit" value="Update" name="bUpdate" class="button"></p><p align="center">
        <a <%=fstatx%> href="EcomReport0.asp">Ecommerce Report</a></p>

<% Else %>

        <p><br></b.r><input type="button" onclick="location.href='javascript:history.back(1)'" value="Return" name="bReturm" class="button"></td>
