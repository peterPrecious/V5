  ...
  <script> 
    ...
    function toggleSave(ele) {
      ...
      if ($("#divNotes")[0].style.display == "block") {
        $("#controlNotes").text("<%=fPhraH(001755)%>");
      } else {
        $("#controlNotes").text("<%=fPhraH(001756)%>");
      }
    }
    ...
    function renderExpandable() {
      if (less) {
        ...
        $("#expandable").text("<%=fPhraH(001757)%>");
      } else {
        $(".expandable").removeClass("less");
        $("#expandable").text("<%=fPhraH(001758)%>");
      }
      less = !less;
    }
  </script>
...
<body>
  ...
  <h1 style="text-align: center"><!--webbot bot='PurpleText' PREVIEW='Learner Report Card'--><%=fPhra(000795)%></h1>
  <div style="text-align: center">
    [ <a href="#" class="green" onclick="toggleSave('divNotes')" id="controlNotes"><!--webbot bot='PurpleText' PREVIEW='Details On/Off'--><%=fPhra(001459)%></a> ] 
    [ <a href="#" class="green" onclick="renderExpandable()" id="expandable"></a> ]
  </div>
  <div id="divNotes" style="text-align: left; margin: 30px;">
    <ul>
      <li><!--webbot bot='PurpleText' PREVIEW='All assessment <b>Score</b>s are shown with the latest score first followed by previous attempts.'--><%=fPhra(001759)%></li>
      <li><!--webbot bot='PurpleText' PREVIEW='<b>Time Spent</b> is the total time in minutes expended while reviewing a module or taking an assessment.<br />Note however, that if the assessment is recurring, ie it must be taken every 6 months, then once completed the Time Spent is reset.'--><%=fPhra(001760)%></li>
      <li><!--webbot bot='PurpleText' PREVIEW='A <b>Closed</b> (&quot;Yes&quot;) session means that a recurring assessment has been completed. If it is not recurring it will show &quot;na&quot;.'--><%=fPhra(001761)%></li>
      <li><!--webbot bot='PurpleText' PREVIEW='A <b>Completed</b> (&quot;Yes&quot;) session is when an assessment is passed or content is deemed completed.'--><%=fPhra(001762)%></li>
      <li><!--webbot bot='PurpleText' PREVIEW='Larger text fields, shown in italics, can be expanded or reduced in size by clicking the &quot;More Text&quot;/&quot;Less Text&quot; link at the top of the page.'--><%=fPhra(001763)%></li>
      <li><!--webbot bot='PurpleText' PREVIEW='You can see any Learner&#39;s Profile by clicking on their'--><%=fPhra(001768)%> <%=fIf(svCustPwd, fPhraH(000411), fPhraH(000211))%><!--webbot bot='PurpleText' PREVIEW='. Note that any changes made to a learner profile will not be reflected on this report unless it is restarted.'--><%=fPhra(001769)%></li>
      <li><!--webbot bot='PurpleText' PREVIEW='Actions:'--><%=fPhra(001765)%>
        <ul>
          <li><!--webbot bot='PurpleText' PREVIEW='<b>&quot;Cert&quot;</b> will generate a certificate for a passed assessment (appears once per module).'--><%=fPhra(001766)%></li>
  ...
  <table class="table">
    <tr>
      <th class="rowshade" style="width: 50px; text-align: center;">
        <!--webbot bot='PurpleText' PREVIEW='Row'--><%=fPhra(001566)%></th>
      <th class="rowshade" style="text-align: left;">
        <!--webbot bot='PurpleText' PREVIEW='Group'--><%=fPhra(000369)%></th>
      <th class="rowshade" style="text-align: left;">
        <!--webbot bot='PurpleText' PREVIEW='Name'--><%=fPhra(000187)%></th>
      <th class="rowshade" style="text-align: left;"><%=fIf(svCustPwd, fPhraH(000411), fPhraH(000211))%></th>
      ...
      <th class="rowshade" style="text-align: left; width: 200px; white-space: nowrap;">
        <!--webbot bot='PurpleText' PREVIEW='Title'--><%=fPhra(000019)%></th>
      <th class="rowshade" style="text-align: left;">
        <!--webbot bot='PurpleText' PREVIEW='Module'--><%=fPhra(000272)%></th>
      <th class="rowshade" style="text-align: left; width: 200px; white-space: nowrap;">
        <!--webbot bot='PurpleText' PREVIEW='Title'--><%=fPhra(000019)%></th>
      <th class="rowshade" style="text-align: center;">
        <!--webbot bot='PurpleText' PREVIEW='Time'--><%=fPhra(000273)%><br>
        <!--webbot bot='PurpleText' PREVIEW='Spent'--><%=fPhra(001394)%></th>
      <th class="rowshade" style="text-align: center;">
        <!--webbot bot='PurpleText' PREVIEW='Score'--><%=fPhra(000232)%></th>
      <th class="rowshade" style="text-align: center;">
        <!--webbot bot='PurpleText' PREVIEW='Date'--><%=fPhra(000112)%></th>
      <th class="rowshade" style="text-align: center;">
        <!--webbot bot='PurpleText' PREVIEW='Completed'--><%=fPhra(000107)%></th>
      <th class="rowshade" style="text-align: center;">
        <!--webbot bot='PurpleText' PREVIEW='Closed'--><%=fPhra(001546)%></th>
