...
<body>
  ...
  <table class="table" style="width: 350px;">
    ...
    <tr>
      <td colspan="<%=vCnt + 3%>">
        <p class="c1"><!--webbot bot='PurpleText' PREVIEW='Programs Purchased and Assigned for'--><%=fPhra(000883)%>&nbsp;<%=custId %></p>
        <p class="c2"><!--webbot bot='PurpleText' PREVIEW='This report displays all learners, sorted by Last Name, who have had programs assigned to them.'--><%=fPhra(000879)%></p>
        <p class="c3"><!--webbot bot='PurpleText' PREVIEW='Summary...'--><%=fPhra(001622)%></p>
      </td>
    </tr>
    <tr>
      <td colspan="3" style="text-align: left;" class="rowshade"><!--webbot bot='PurpleText' PREVIEW='Programs'--><%=fPhra(000203)%></td>
      <% For i = 1 To vCnt %>
      <td class="rowshade" style="text-align: center; width: 50px;"><a title="<%=fProgTitleClean(aProgs(1, i))%>" href="#"><%=aProgs(1, i)%></a></td>
      <% Next %>
    </tr>
    <tr>
      <td colspan="3" style="text-align: left;" class="rowshade"><!--webbot bot='PurpleText' PREVIEW='Purchased Total'--><%=fPhra(000880)%></td>
      <% For i = 1 To vCnt %>
      <td class="rowshade" style="text-align: center;"><%=aProgs(2, i)%></td>
      <% Next %>
    </tr>
    <tr>
      <td colspan="3" style="text-align: left;" class="rowshade"><!--webbot bot='PurpleText' PREVIEW='Assigned Total'--><%=fPhra(000881)%></td>
      <% For i = 1 To vCnt %>
      <td class="rowshade" style="text-align: center;"><%=aProgs(0, i)%></td>
      <% Next %>
    </tr>
    <tr>
      <td colspan="3" style="text-align: left;" class="rowshade"><!--webbot bot='PurpleText' PREVIEW='Balance Remaining'--><%=fPhra(000882)%></td>
      <% For i = 1 To vCnt %>
      <td class="rowshade" style="text-align: center;"><%=aProgs(2, i) - aProgs(0, i)%></td>
      <% Next %>
    </tr>
    <tr>
      <td colspan="3" style="border-bottom:1px solid navy"><p class="c3"><!--webbot bot='PurpleText' PREVIEW='Assigned...'--><%=fPhra(000884)%></p></td>
    </tr>
    ...
    <tr>
      <th style="text-align: left" colspan="3" class="rowshade"><!--webbot bot='PurpleText' PREVIEW='Programs'--><%=fPhra(000203)%></th>
      <% For i = 1 To vCnt %>
