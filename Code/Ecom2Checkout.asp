...
<body>
  ...
  <% 
    ...
    '...determine source, especially if member bypasses ecommerce
    vEcom_Source = "E" '...normal ecom
    If Len(svMembNo) > 0 Then 
      sGetMemb (svMembNo)      
      If Session("EcomBypass") = True Then '...set in SignIn.asp (for Boreal)
         vEcom_Source = "C"   
      ElseIf vMemb_Ecom Then '...using ecom bypass
  ...
  <table class="table">
    <tr>
      <td style="text-align: center"><h6><br><!--webbot bot='PurpleText' PREVIEW='There are no Programs to Checkout.'--><%=fPhra(000263)%><br></h6></td>
    </tr>
  </table>
  <%
    Else
  ...
  <table class="table">
    <tr>
      <td colspan="3">
        <table class="table">
          <tr>
            <td>
              <!--<script>jTitle("<%=fPhraH(000916)%>", 'Report.jpg')</script>-->
    ...
    %>
    <tr>
      <td><!--webbot bot='PurpleText' PREVIEW='GST'--><%=fPhra(000159)%></td>
      <td><!--webbot bot='PurpleText' PREVIEW='Tax'--><%=fPhra(000245)%>@ <%=FormatPercent(fGST(Now, xxxCountry, xxxProvince))%></td>
    ...
    %>
    <tr>
      <td><!--webbot bot='PurpleText' PREVIEW='PST'--><%=fPhra(000264)%></td>
      <td><!--webbot bot='PurpleText' PREVIEW='Tax'--><%=fPhra(000245)%>@ <%=FormatPercent(fPST(Now, xxxCountry, xxxProvince))%></td>
    ...
    %>
    <tr>
      <td><!--webbot bot='PurpleText' PREVIEW='HST'--><%=fPhra(000146)%></td>
      <td><!--webbot bot='PurpleText' PREVIEW='Tax'--><%=fPhra(000245)%>&nbsp; @ <%=FormatPercent(fHST(Now, xxxCountry, xxxProvince))%></td>
    ...
    <tr>
      <td colspan="3">
        <form action="<%=vEcomURL%>" method="POST" target="_top">
          ...
          <input type="hidden" name="ssl_phone" value="<%=ssl_phone%>">
