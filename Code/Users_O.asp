...
<body>
	...
	<p class="c3">
		<!--webbot bot='PurpleText' PREVIEW='Note:&nbsp; <b>Expires</b> will be blank unless the Learner was setup manually or via e-commerce.&nbsp; The <b>Group|Rights</b> filters will be empty unless configured.&nbsp; Click on the'--><%=fPhra(000375)%> <b><%=fIf(svCustPwd, fPhraH(000411), fPhraH(000211))%></b> <!--webbot bot='PurpleText' PREVIEW='to modify the Learner&#39;s profile.'--><%=fPhra(000368)%>&nbsp; 
		<% If bG2CustEmail Then %>
			<!--webbot bot='PurpleText' PREVIEW='If a learner did not receive their &quot;Welcome&quot; email alert, for whatever reason, click the &quot;Resend&quot; button beside that learner's name.'--><%=fPhra(001773)%>
		<% End If %>       
	</p>
	...
	<table class="table">
		<tr>
			<th class="rowShade" style="text-align:left; width:150px"><!--webbot bot='PurpleText' PREVIEW='Group'--><%=fPhra(000369)%><% If svMembLevel = 5 Then %><br><!--webbot bot='PurpleText' PREVIEW='Rights'--><%=fPhra(000230)%><% End If %></th>
			<th class="rowShade" style="text-align:left; width:200px"><!--webbot bot='PurpleText' PREVIEW='Name'--><%=fPhra(000187)%>, <!--webbot bot='PurpleText' PREVIEW='Organization'--><%=fPhra(000470)%><br><!--webbot bot='PurpleText' PREVIEW='Email Address'--><%=fPhra(000126)%></th>
			<th class="rowShade" style="text-align:center; width:100px"><!--webbot bot='PurpleText' PREVIEW='Active'--><%=fPhra(000063)%>?<%=fIf(svMembLevel=5, "<br />History", "")%></th>
			<th class="rowShade" style="text-align:left; width:200px"><%=fIf(vGlobal="1","(CustId) ","")%> <%=fIf(svCustPwd, fPhraH(000411), fPhraH(000211))%><br><!--webbot bot='PurpleText' PREVIEW='Memo'--><%=fPhra(000173)%></th>
			<th class="rowShade" style="text-align:center; width:100px"><!--webbot bot='PurpleText' PREVIEW='First Visit'--><%=fPhra(000157)%><br /><!--webbot bot='PurpleText' PREVIEW='Last Visit'--><%=fPhra(000164)%></th>
			<th class="rowShade" style="text-align:center; width:100px"><!--webbot bot='PurpleText' PREVIEW='Expires'--><%=fPhra(000137)%></th>
			<th class="rowShade" style="text-align:center; width:100px"># <!--webbot bot='PurpleText' PREVIEW='Site Visits'--><%=fPhra(001382)%><br><!--webbot bot='PurpleText' PREVIEW='Total Mins'--><%=fPhra(001787)%><br /><!--webbot bot='PurpleText' PREVIEW='Online'--><%=fPhra(000488)%></th>
			<% If bG2CustEmail Then %>
			<th class="rowShade" style="text-align:center; width:100px"><!--webbot bot='PurpleText' PREVIEW='Resend'--><%=fPhra(001772)%><br><!--webbot bot='PurpleText' PREVIEW='Email Alert'--><%=fPhra(000127)%></th>
			<% End If %>
		</tr>
		<tr>
			<td colspan="<%=vCols%>">&nbsp;</td>
	...
	<form method="POST" action="Users_O.asp">
		...
		<div style="margin:auto; text-align:center;">
			...
			<% End If %>
				...
				<a href="<%=vEdit%>?vMembNo=0&vNext=<%=vNext%>&vCustId=<%=vCustId%>"><!--webbot bot='PurpleText' PREVIEW='Add a Learner'--><%=fPhra(000370)%></a>
				<% End If %>
