  ...
  <table border="1" width="100%" bordercolor="#DDEEF9" style="border-collapse: collapse" cellpadding="0" cellspacing="0">
    <tr>
      <td align="center">
        ...
        <h2>
        <% If vJustViewingCert Then %>
          <!--webbot bot='PurpleText' PREVIEW='Your highest achieved Certificate is now displayed in a separate window.'--><%=fPhra(000053)%><br>
        <% Else %>
        ...
        %>
          <!--webbot bot='PurpleText' PREVIEW='This Certificate may now be printed by pressing &lt;Ctrl&gt;+p simultaneously.'--><%=fPhra(000006)%><br>
        <%   Else %>
          <!--webbot bot='PurpleText' PREVIEW='This Certificate may now be printed by pressing &lt;Command&gt;+p simultaneously.'--><%=fPhra(000005)%><br>&nbsp; </h2>
        <% End If %>
  
        <h2>
        
        <a <%=fstatx%> href="javascript:jCertificate('<%=svLang%>','','','','','Exam', '')">
        <!--webbot bot='PurpleText' PREVIEW='Click here if your certificate did not appear in a separate window.'--><%=fPhra(000098)%></a>
        <br>
  
        <a <%=fstatx%> href="#" onclick="fullScreen('<%=fCertificateUrl("", "", vMark, "", vModId, Session("CertTitle"), "", "", "", vProg_Id, "", "", "")%>')">
        <!--webbot bot='PurpleText' PREVIEW='Click here if your certificate did not appear in a separate window.'--><%=fPhra(000098)%></a> [New Certificate]
  
        &nbsp;&nbsp;&nbsp; 
        <% If vMark <> 1 Then %> 
          <br><br><br>
          <!--webbot bot='PurpleText' PREVIEW='Click below to review the Questions that were answered incorrectly.'--><%=fPhra(000091)%> </h2>
          <p align="center">&nbsp;
