...
'   Session("MembId")     = "WebService"
    ...
    For k = 0 to Ubound(aMods)
      ...
      If vMods_Active Then
        ...
        If fModsDesc (vMods_Id) Then
          vWs = vWs & " [<a " & fStatX & " href=""javascript:SiteWindow('ModuleDescription.asp?vClose=Y&vModId=" & vMods_Id & "')"" title='" & vUrlTitle1 & "'>" & Server.HtmlEncode(fPhraH(000118)) & "</a>]"
        End If
    ...
    If Len(Trim(vProg_Assessment)) > 0 Then  
      ...
      vWs = vWs &          fVuAssessLink (vProg_Assessment, fPhraH(000132), Server.HtmlEncode(vMods_Title))
      vWs = vWs & "      </td>"
      vWs = vWs & "    </tr>" & vbCrLf
...
'    ElseIf Lcase(vProg_Exam) <> "n" Then  
     ElseIf Len(vProg_Exam) > 1 Then  
      ...
      vWs = vWs & "    <tr>" & vbCrLf
      vWs = vWs & "      <td width='550'>&ensp;&ensp;&ensp;<span class='c2'><a " & fStatX & " href=""javascript:examwindow('" & vProg_Exam & "')"" title='" & vUrlTitle1 & "'>" & fPhraH(000132) & "</a></span>"
